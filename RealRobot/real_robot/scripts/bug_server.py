...
class Navigator:
    ...
    def _get_front_left_right_scan_max_ranges(self, theta_1, theta_2, R, x_lim):
        ...
        n_points = int(np.ceil((theta_2 - theta_1) / self.angle_step))
    ...
    def _get_front_left_right_scan_slices(self, offset=-90):
        ...
        theta2_idx = theta2_idx + 1 if len(self.max_front_distances) > (theta2_idx - theta1_idx) else theta2_idx
        theta2_idx = theta2_idx - 1 if len(self.max_front_distances) < (theta2_idx - theta1_idx) else theta2_idx
   
        pi_idx = pi_idx + 2 if len(self.max_left_distances) > (pi_idx - theta2_idx - 1) else pi_idx
        pi_idx = pi_idx - 1 if len(self.max_left_distances) < (pi_idx - theta2_idx - 1) else pi_idx
        ...
        min_length = max(len(range(left_slice.start, left_slice.stop)), len(range(right_slice.start, right_slice.stop)))
...
def handle_bug(req):
    ...
    if req.bug == 0:
        ...
        navigator.go_to_point(req.point)
...
if __name__ == '__main__':
    ...
    rospy.loginfo("Bug Servie is ready")
